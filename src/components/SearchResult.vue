...
const selectPage = computed(() => {
  ...
  if (holeSearchValue.value.page <= 3) {
    ...
    return temp.filter(
      (val) =>
        val <= holeSearchValue.value.page + 2 &&
        val >= holeSearchValue.value.page - 2
    );
  }
});
...
// 破壞程度_資料重構
const dtypeRebuild = (dt = "-") => (dt.includes("S") ? "S" : dt);
...
const changePage = (e) => {
  ...
  // 是 上一頁 & 下一頁
    ...
    switch (type) {
      case "+":
        if (holeSearchValue.value.page !== user.totalPages)
          holeSearchValue.value.page++;
        break;
      case "-":
...
<template>
  <div
    ...
    <table>
      ...
      <tbody>
        <tr
          v-for="(el, i) in user.resultObj"
          :key="i"
          @click="detailOnClick(el)">
