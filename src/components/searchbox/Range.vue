...
const confirmSearch = () => {
  ...
  // axios
  axios
    .post("http://211.72.231.157/Kcg_Wrb_SP/api/v1/search_TRACE_List", body())
    .then((res) => {
      let exist = [];
      res.data.data.count === 0
        ? user.alertShowing()
        : (exist = res.data.data.data);
      return exist;
    })
...
<template>
  <div class="range">
    <div
      class="inputWrap"
      v-for="(e, index) in inputs"
      :key="index">
      <label :for="e.type">{{ e.label }}</label>
      <el-select
        v-if="e.type === 'option'"
        v-model="value[e.value]"
        :placeholder="`請選擇${e.label}`"
        size="large"
        style="width: 100%">
        <el-option
          v-for="item in e.store"
          :value="item.value"
          :label="item.label"
          :key="item.label" />
      </el-select>
